...
#     # states = json.loads(state_response.text)
...
#     states_dict = {}
#     states_dict['0'] = 'Andaman and Nicobar Islands'
...
#     states_list = list(states_dict.values())
    ...
    #     return "key doesn't exist"
        ...
        #     #district = json.loads(district_response.text)
        #     if district_response.status_code != 200:
        #         return district_response.json()
        ...
        #     return district_dict
            ...
            #                 # Fetch Center
            #                 center_response = requests.get(
            #                 f"https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/calendarByDistrict?district_id={dist_key}&date={new_date}",headers=header)
            #                   # f"https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/calendarByDistrict?district_id=329&date=19-06-2022",headers=header)
            #                 #centers_data =  json.loads(center_response.text)
            #                 if center_response.status_code != 200:
            #                         return center_response.json()
            #                 centers_data = center_response.json()
            #                 centers = pd.DataFrame(centers_data.get('centers'))
            ...
            #                     new_df.columns = ['Center_ID', 'Name', 'Address', 'Pincode','Fee','Minimum Age', 'Vaccine Type', 'Dose 1','Dose 2','Timing']
            #                     new_df = new_df[['Center_ID', 'Name', 'Fee','Pincode',
            #                                       'Minimum Age', 'Vaccine Type', 'Timing', 'Address','Dose 1','Dose 2']]
            #                     if new_df.empty:
            #                         st.error("No Center found.")
            #                     else:
            #                         st.dataframe(new_df.assign(hack='').set_index('hack'))
            ...
            #                         href = f'<a href="https://selfregistration.cowin.gov.in/">Book ,</a>'
            #                         st.markdown(href,unsafe_allow_html=True)
